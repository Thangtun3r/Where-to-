...
namespace Yarn.Unity
{
    ...
    public sealed class BespokeOptionsPresenter : DialoguePresenterBase
    {
        [SerializeField] CanvasGroup? canvasGroup;

        [SerializeField] bool autoRefreshOptionsOnStart = true;

        // A cached pool of OptionView objects so that we can reuse them
        List<BespokeOptionItem> optionViews = new List<BespokeOptionItem>();
        ...
        {
            // If we don't have any views, try to find them now
            ...
            {
                Debug.LogWarning($"There are more options ({dialogueOptions.Length}) than there are {nameof(OptionItem)} children ({optionViews.Count}). Some options will not be shown.", this);
            }
            ...
            // Force the layout to rebuild now that we've toggled the children
